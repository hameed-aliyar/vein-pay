...
from django.contrib.postgres.fields import ArrayField  # or use JSONField
from django.db import models
import decimal
...
class BiometricData(models.Model):
    ...
    owner = models.OneToOneField(User, on_delete=models.CASCADE, related_name="biometric_data")
    biometric_type = models.CharField(max_length=10, choices=BIOMETRIC_CHOICES)
    
    # For FACE
    face_template = models.ImageField(upload_to="face_templates", blank=True, null=True)

    
    # new column for embeddings
    vein_embedding_json = models.JSONField(blank=True, null=True)
